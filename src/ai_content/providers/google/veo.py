...
class GoogleVeoProvider:
    ...
    async def generate(
        ...
        *,
        ...
        try:
            # Build config
            # Note: person_generation is often restricted or defaults to strict filtering.
            # We omit it if it causes issues, or only include it if explicitly supported.
            config_params = {"aspect_ratio": aspect_ratio}
            if person_generation not in ["allow_adult", "allow_adult_faces"]:
